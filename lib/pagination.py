...
def _entity_ids_fingerprint(entities):
    """Stable-ish fingerprint to detect repeated pages when API ignores page cursor."""
    if not entities:
        return ()
    ids = []
    for row in entities:
...
def paginate_post(air_host, api_token, path, body, params=None, page_size=100, verbose=True):
    """
    Paginate a POST endpoint that returns the same Binalyze result.entities shape as GET.

    Sends the same JSON `body` on every page; adds page/pageSize as query parameters.
    """
    base_params = dict(params or {})
    all_entities = []
    page = 1
